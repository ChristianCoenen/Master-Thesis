...
class NetworkSupervised(Network):
    ...
    def __init__(
        ...
        :param encoder_latent:
            Whether the encoder receives random inputs to generate random latent spaces or random latent spaces are
            directly used.
    ...
    def generate_latent_and_classification_points(self, n_samples: int):
        """Generates random latent spaces (through the encoder) and random classification labels

        :param n_samples:
            Number of samples that needs to be generated.
        :return:
            x_latent: Latent space values generated by either randomly generated using a normal distribution or
                      by feeding random inputs to the encoder
            x_classification: Random classification labels (one for each latent space array)
        """

        # generate random points in the latent space
        if self.encoder_latent:
    ...
    def generate_fake_samples(self, n_samples: int):
        ...
        :param n_samples:
            Number of samples that needs to be generated.
        :return:
            fake_samples: Samples generated by the decoder (generator)
            y: Array with length of n_samples containing zeros (to indicate that those are fake samples)
            x_classification: Array with length of n_samples showing the objective for the decoder for each sample
               Helps showing whether the generated samples match the classification input (e.g. generate a 6)
        """
        # generate random points in the latent space
        x_latent, x_classification = self.generate_latent_and_classification_points(n_samples)
        # predict outputs
        fake_samples = self.decoder.predict([x_latent, x_classification])
        # create 'fake' class labels (0)
        y = zeros((n_samples, 1))
        return fake_samples, y, x_classification

    def generate_real_samples(self, n_samples: int):
        ...
        :return:
            ...
            y: Array with length of n_samples containing zeros (to indicate that those are fake samples)
            labels: Array with length of n_samples containing the matching labels for the samples in x
        """
    ...
    def summarize_performance(self, epoch: int, n_samples: int = 100):
    ...
    def evaluate(self):
        """Evaluates the autoencoder based on the test data (only returns classification accuracy,
        cause decoder accuracy is not valuable (interpreting result as visualized image is better)
        """
    ...
    def save_model_architecture_images(
            self, models: Optional[List[Model]] = None, path: str = "supervised/images/architecture",
            fmt: str = "png"
    ):
    ...
    def create_modified_classification_plot(
            self,
            sample_idx: int = None,
            path: str = "supervised/images/plots",
            random: bool = False
    ):
        ...
        :param random:
            If set to true and no sample_idx provided, a random sample is used
        :return:
            None
        """
    ...
    def save_reconstruction_plot_images(
            self,
            samples,
            state: str,
            path: str = "supervised/images/plots",
    ...
    def save_fake_sample_plot_images(
            self, x_fake=None, labels=None, epoch: int = -1, n_samples: int = 100, path: str = "supervised/images/plots"
    ):
        """Create and save a plot of generated images (reversed grayscale)
        Useful to show if the generator is able to generate real looking images from random points.

        :param x_fake:
            Fake samples that are plotted
        :param labels:
            Labels for the provided fake samples. Number of samples and labels has to be the same!
        :param epoch:
            Current training epoch (used to name the file)
        :param n_samples:
