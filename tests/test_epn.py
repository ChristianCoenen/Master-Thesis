...
def test_shared_weights():
    epn = EntropyPropagationNetwork(
        dataset="mnist", weight_sharing=True, encoder_dims=[100], latent_dim=40, classification_dim=10
    )
    epn.train(epochs=1)
...
def test_epn_with_weight_sharing_mnist():
    ...
    epn.train_autoencoder(epochs=1)
...
def test_epn_without_weight_sharing_mnist():
    ...
    d_threshold = 0.75
    epn = EntropyPropagationNetwork(
        dataset="mnist", weight_sharing=False, encoder_dims=[100], latent_dim=40, classification_dim=10
    )
    epn.train_autoencoder(epochs=1)
    _, _, _, c_acc, d_acc = epn.evaluate()
    assert c_acc > c_threshold, f"Classification accuracy: {c_acc}% | Threshold: {c_threshold}%"
...
def test_epn_with_weight_sharing_fashion_mnist():
    ...
    d_threshold = 0.45
    epn = EntropyPropagationNetwork(
        dataset="fashion_mnist", weight_sharing=True, encoder_dims=[100], latent_dim=40, classification_dim=10
    )
    epn.train_autoencoder(epochs=1)
    _, _, _, c_acc, d_acc = epn.evaluate()
    assert c_acc > c_threshold, f"Classification accuracy: {c_acc}% | Threshold: {c_threshold}%"
...
def test_epn_with_multiple_encoder_decoder_layers():
    ...
    d_threshold = 0.75
    epn = EntropyPropagationNetwork(
        dataset="mnist", weight_sharing=True, encoder_dims=[400, 200, 100], latent_dim=40, classification_dim=10
    )
    epn.train_autoencoder(epochs=1)
    _, _, _, c_acc, d_acc = epn.evaluate()
    assert c_acc > c_threshold, f"Classification accuracy: {c_acc}% | Threshold: {c_threshold}%"
...
def test_epn_with_huge_latent_space():
    ...
    d_threshold = 0.75
    epn = EntropyPropagationNetwork(
        dataset="mnist", weight_sharing=True, encoder_dims=[100], latent_dim=400, classification_dim=10
    )
    epn.train_autoencoder(epochs=1)
    _, _, _, c_acc, d_acc = epn.evaluate()
    assert c_acc > c_threshold, f"Classification accuracy: {c_acc}% | Threshold: {c_threshold}%"
...
def test_epn_with_tiny_latent_space():
    ...
    d_threshold = 0.75
    epn = EntropyPropagationNetwork(
        dataset="mnist", weight_sharing=True, encoder_dims=[100], latent_dim=3, classification_dim=10
    )
    epn.train_autoencoder(epochs=1)
    _, _, _, c_acc, d_acc = epn.evaluate()
    assert c_acc > c_threshold, f"Classification accuracy: {c_acc}% | Threshold: {c_threshold}%"
