...
class EPNetworkRL(EPNetwork):
    ...
    def __init__(
        ...
        self.env = env
        self.env.observation_space.shape = (96, 96)
        self.flat_ob_space = np.prod(np.array(env.observation_space.shape))
        self.classification_dim = self.flat_ob_space + env.action_space.shape[0] + 1
        ...
        self.encoder, self.decoder, self.autoencoder = self.build_autoencoder(
            encoder_input_tensors=[
                Input(shape=env.observation_space.shape, name="state"),
                Input(shape=env.action_space.shape[0], name="action"),
            ],
            encoder_output_layers=[
                Dense(1, activation="tanh", name=f"expected_reward"),
                Dense(self.flat_ob_space, activation="softmax", name=f"expected_next_state"),
        ...
        #     input_tensors=[
        #         Input(shape=env.observation_space.shape, name="reconstructed_state"),
        #         Input(shape=env.action_space.shape[0], name="recustructed_action"),
        #     ],
        ...
        #     input_tensors=[
        #         Input(shape=env.observation_space.shape, name="state"),
        #         Input(shape=env.observation_space.shape, name="expected_next_state"),
        #         Input(shape=1, name="expected_reward"),
        #         Input(shape=env.action_space.shape[0], name="reconstructed_action"),
        #     ],
        #     output_layers=[Dense(1, activation="sigmoid", name="real_or_fake")],
    ...
    def get_episodes_from_dataset(self, n: int, random: bool):
        ...
        return env_state, action, reward, next_env_state

    def generate_new_states(self, n: int):
        position_arr = np.empty((n, self.env.maze.size[0] + self.env.maze.size[1]), dtype=int)
        env_state_arr = np.zeros((n, self.env.observation_space.shape), dtype=int)
        action_arr = np.empty((n, self.env.action_space.shape[0]), dtype=int)
    ...
    def generate_random_episodes(self, get_obj, n):
        if get_obj:
            ...
            env_state = np.zeros((n, self.env.observation_space.shape), dtype=int)
            next_env_state = np.zeros((n, self.env.observation_space.shape), dtype=int)

        action = np.empty((n, self.env.action_space.shape[0]), dtype=int)
    ...
    def visualize_outputs_to_file(self, state, n_samples=9, path="images/epn_rl/plots"):
        ...
        # save_plot_as_image(path=path, filename=f"car_racing_{state}", dpi=300 + self.nr_tiles * 5)
